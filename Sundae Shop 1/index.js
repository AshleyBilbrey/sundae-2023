...
app.post('/', (req, res) => {
  try {
    ...
    for (const item in req.body) {
      ...
      if(!cart.total) {
        cart.total = Number(req.body[item] * flavorList[item].price);
      } else {
        cart.total = Number(cart.total) + Number(req.body[item] * flavorList[item].price);
      }
...
function getCartInfo(cookie) {
  console.log(cookie)
  if(!cookie) return {items: {}, total: 0.0, numItems: 0}
...
app.get("/checkout", (req, res) => {
  ...
  if(cart.total > 49.99) {
    res.render("checkout.ejs", {message: "The transaction limit is $49.99. Your cart has been cleared."})
  } else {
    if(cart.items.flag && cart.items.flag > 0) {
      res.render("checkout.ejs", {message: "Thank you for your order. moo{g3t_ur_1c3_cr34m_n0mn0m}"})
    } else {
